"""
DJI_Image_Processor.pyw – GUI front-end for DJI drone image/video processing tools.

Tabs:
  - DJI Thermal Converter        : converts R-JPEG thermal images to float32 TIFF (mode 1)
  - Geotagged Video Frame Extractor : extracts geotagged frames from DJI videos via SRT files (mode 2)
  - Batch Image Coordinate Editor   : applies a GPS altitude offset to a folder of images (mode 3)

Each tab spawns elaborator.py as a subprocess using the current Python interpreter.
"""
import sys
import os
...
def popup():
    """Copy the contact email to the clipboard and show a confirmation dialog."""
...
def popup_folder():
    """Show an error dialog when the user tries to process without selecting a folder first."""
...
def resource_path(relative_path):
    """Return absolute path to a resource relative to this script, compatible with PyInstaller."""
    try:
...
def start_batch_video():
    """Launch elaborator.py in mode 2 to extract geotagged frames from all videos in the selected folder."""
    if entry_1.get() == "":
        al = "0"
    else:
        al = entry_1.get()
    ...
    if folder_path_video.get() == "No Folder Selected":
        ...
        subprocess.Popen([sys.executable, resource_path("elaborator.py"), folder_path_video.get(), "2", al, fps, str(switch_quality.get())])  # , drone_var.get()])


def start_batch_image():
    """Launch elaborator.py in mode 3 to apply a GPS altitude offset to all images in the selected folder."""
    ...
    if folder_path_image.get() == "No Folder Selected":
        ...
        subprocess.Popen([sys.executable, resource_path("elaborator.py"), folder_path_image.get(), "3", al_im])


def start_batch_flir():
    """Launch elaborator.py in mode 1 to convert DJI thermal R-JPEG images to float32 TIFF."""
    if emissivity.get() == "":
        em = "0.95"
    else:
        em = emissivity.get()
    ...
    if folder_path_thermal.get() == "No Folder Selected":
        ...
        subprocess.Popen([sys.executable, resource_path("elaborator.py"), folder_path_thermal.get(), "1", em, dist, hu, ref, str(switch.get()),
                          str(switch2.get())])


def browse_folder_video():
    """Open a directory picker and store the result in folder_path_video."""
...
def browse_folder_image():
    """Open a directory picker and store the result in folder_path_image."""
...
def browse_folder_thermal():
    """Open a directory picker and store the result in folder_path_thermal."""
